...
if (state == SAVINGFX)
{
    if (statetime == 1)
    {
        ...
        save_game("diffusion/save" + (string(global.saveslot + 1)))
        refill_heath_ammo()
    }
    if (statetime == 230)
        state = IDLE
...
if ((chargebeam > 0 && chargebeam < 0.03) || (diffcharge > 0 && diffcharge < 0.03))
    sfx_play(sndCharge)
if ((chargebeam > 0.97 && chargebeam < 1) || (diffcharge > 0.97 && diffcharge < 1))
{
    LoopSoundMono(sndChargeLoop)
    sfx_stop(sndCharge)
}
if (chargebeam == 0 && diffcharge == 0)
{
    sfx_stop(sndCharge)
